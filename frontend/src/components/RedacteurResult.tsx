...
import type { RedacteurResultData, Tonalite } from "../api/types";

const TONALITE_CLASSES: Record<Tonalite, string> = {
  prudent: "text-blue-400 ",
  equilibre: "text-yellow-400 ",
  assertif: "text-red-500 ",
};
...
function DraftCard({
  ...
  return (
    <div className="relative flex flex-col gap-2 rounded-lg border border-[var(--border)] p-4">
      ...
      <span
        className={`text-xs uppercase tracking-wide text-[var(--text)] ${TONALITE_CLASSES[draft.tonalite]}`}
      >
        {draft.tonalite}
      </span>
      ...
      <button
        type="button"
        onClick={handleCopy}
        className="mt-auto self-start rounded-lg border border-[var(--border)] px-3 py-1 text-sm transition hover:bg-[var(--code-bg)]"
      >
        {copied ? "Copié ✓" : "Copier"}
      </button>
    </div>
